...
import { paintInfoState, tempPaintInfoState, undoListState } from 'src/state/paintInfoState';
import { Buttons } from '../Paint.style'

const RelatedPaintInfoBtn = () => {
  const [paintInfo, setPaintInfo] = useRecoilState(paintInfoState);
  const [tenoPaintInfo, setTempPaintInfo] = useRecoilState(tempPaintInfoState);
